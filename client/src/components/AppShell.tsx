...
export function AppShell({ children, admin = false }: { children: ReactNode; admin?: boolean }) {
  ...
  if (admin) return <div className="admin-shell"><SiteHeader />{artifacts.map((artifact) => <PageArtifactView key={artifact.id} artifact={artifact} onClose={() => removeArtifact(artifact.id)} />)}{children}<AgentChat /></div>;
